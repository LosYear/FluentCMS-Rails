module Fluent
  class Datagrids::BlocksGrid
    ...
    column(:name, :header => I18n.t('activerecord.attributes.fluent/block.name'), :html => true) do |record|
      link_to record.name, edit_admin_block_path(record), :data => {:push => true}
    end
    column(:title, :header => I18n.t('activerecord.attributes.fluent/block.title'))
    column(:actions, :html => true, :header => I18n.t('backend_part.actions')) do |record|
      ...
      link_to(icon('trash'), admin_block_path(record), 'data-original-title' => t('backend_part.remove'), 'id' => 'tooltip',  :method => :delete, :confirm => "#{t("backend_part.confirm")}")
    end
